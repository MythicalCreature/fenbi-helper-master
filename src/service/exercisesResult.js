...
exports.getQuestionByIds = async function (questionIds,cookie) {
...
exports.getQuestionMetaByIds = async function (questionIds,cookie) {
    let questions = await httpRequest({
...
exports.getQuestionKeyPointsByIds = async function (questionIds,cookie) {
    let questions = await httpRequest({
...
async function getSolutionsByIds(questionIds,cookie) {
    let solutions = await httpRequest({
    ...
    return _.zipObject(solutions.map(q => q.id), solutions)
}
// 返回收藏了的题目的id数组
...
exports.getCollectsByIds = getCollectsByIds;

async function getCollects(cookie) {
    return await httpRequest({
        url: `https://tiku.fenbi.com/api/xingce/collects`,
        method: "GET",
        json: true,
        headers: {
            ...headers,
...
// 假设这是 exerciseResult.getExerciseExp 的内部实现（伪代码）
exports.getExerciseExp = async function (exerciseId, cookie) {
    try {
        return getExercise(exerciseId, cookie);
    } catch (err) {
        // ❗ 错误处理：确保 reject 的是一个 Error 对象
        console.error('获取题目失败:', err);
        reject(new Error(`获取题目失败，原因：${err.message || '未知错误'}`));
        // 或者更详细的自定义 Error
        // const error = new Error('获取题目失败');
...
async function getExerciseHistory(categoryId, cookie) {
    ...
    let hisArr = await Promise.all(cursorArr.map(cursor => {
        return httpRequest({
            ...
            //https://tiku.fenbi.com/api/xingce/category-exercises?categoryId=1&cursor=0&count=15&noCacheTag=813&app=web&kav=100&av=100&hav=100&version=3.0.0.0
            method: "GET",
            json: true,
            headers: {
                ...headers,
...
exports.getSolutionsByIds = async function (questionIds, cookie) {
    let questions = await httpRequest({
        url: `https://tiku.fenbi.com/api/xingce/solutions?ids=${questionIds.join(',')}`,
        method: "GET",
        json: true,
